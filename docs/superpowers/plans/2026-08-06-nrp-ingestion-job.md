...
**Actual result (this run):** Job `Complete`, 160 papers / 8,299 chunks / 58,458 edges in NRP Postgres — substantially larger, as expected. However, the "the old 117 are naturally re-covered, no separate migration needed" premise in the design spec's Context section turned out to be 95%, not 100%, true: only 111 of the original 117 papers are present in the new NRP storage. 6 previously-fetchable papers failed to re-download on this run (transient failures, DOIs: `10.1038/nature13793`, `10.1038/s41579-018-0029-9`, `10.1038/s41591-019-0458-7`, `10.1080/03014460.2025.2509606`, `10.1101/gr.186072.114`, `10.1101/gr.213959.116`). Their PDFs/markdown still exist on Cosmos scratch, just not in NRP storage. See `k8s/nrp/README.md`'s "Known gap: NRP corpus is not a strict superset of the old Cosmos corpus" section for the full note. This is a known, accepted gap, not something this plan re-opens or attempts to close.
